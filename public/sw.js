...
function localUrl(path) {
  return new URL(path.replace(/^\//, ""), scopeRoot()).href;
}
...
async function cacheInBatches(cache, urls, size = 8) {
  for (let index = 0; index < urls.length; index += size) {
    const batch = urls.slice(index, index + size);
    await Promise.all(batch.map((url) => cacheRequired(cache, url)));
  }
}
...
async function precache() {
  ...
  const coreAssets = [
    ...
    localUrl("brand-mark.svg"),
    localUrl("icons/icon-192.png"),
    localUrl("icons/icon-512.png"),
    localUrl("icons/maskable-512.png"),
    localUrl("apple-touch-icon.png"),
    localUrl("plans/mcheyne-classic.v1.json"),
  ];
  ...
  const scriptureAssets = [
    localUrl(bibleManifest.searchIndexPath),
    ...bibleManifest.books.map((book) => localUrl(book.path)),
  ];
  ...
  if (request.mode === "navigate") {
    event.respondWith((async () => {
      try {
        ...
        if (response.ok) {
          const cache = await caches.open(CACHE_NAME);
          await cache.put(scopeRoot().href, response.clone());
        }
        return response;
      } catch {
        const cached = await caches.match(scopeRoot().href);
