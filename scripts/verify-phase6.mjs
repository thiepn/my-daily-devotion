...
assert.match(prayerScreen, /PRAYER_DEPTH_TARGETS/); assert.match(prayerScreen, /Session length/); assert.match(newPrayer, /What do you want to pray about/); assert.match(newPrayer, /sourceReflectionId/); assert.match(detail, /Updates & encouragements/); assert.match(detail, /Mark answered/); assert.match(session, /Next/); assert.match(session, /Skip/); assert.match(context, /buildPrayerFromScriptureUrl/);
