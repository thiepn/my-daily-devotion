...
assert.match(bible, /buildPrayerFromScriptureUrl/); assert.match(bible, /onClick=\{openPrayer\}/); assert.doesNotMatch(bible, /Prayer capture arrives in Phase 6/);
assert.match(today, /TodayPrayerPanel/); assert.doesNotMatch(today, /Focused prayer becomes functional in Phase 6/);
assert.match(reflection, /Create prayer →/); assert.doesNotMatch(reflection, /Phase 6 will turn this into a prayer/);
...
console.log("  Bible/Reflection/Today integration + basic least-recently-prayed focused flow wired locally");
