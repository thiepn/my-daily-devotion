...
assert.match(history, /aria-label="Previous month"/); assert.match(history, /aria-label="Next month"/); assert.match(history, /aria-label=\{`Open history for \$\{dateLabel\(date\)\}`\}/); assert.match(history, /aria-current=/);
assert.match(schema, /DATABASE_SCHEMA_VERSION = 1/);
assert.match(tokens, /--color-ink-faint:\s*#696b62/);
assert.match(vite, /manifest:\s*true/);
assert.match(sw, /\.vite\/manifest\.json/); assert.match(sw, /Object\.values\(buildManifest\)/); assert.match(sw, /entry\.file/);
...
console.log("  complete lazy-route precache + controlled cold-offline Scripture/search journey certified");
