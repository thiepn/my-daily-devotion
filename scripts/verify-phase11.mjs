...
const [pkgRaw, config, core, accessibility, responsive, pwa, app, today, plan, history, schema, phase11Doc] = await Promise.all([
  read("package.json"),
  read("playwright.config.ts"),
  read("tests/ux/core-flow.spec.ts"),
  read("tests/ux/accessibility.spec.ts"),
  read("tests/ux/responsive.spec.ts"),
  read("tests/ux/pwa.spec.ts"),
  read("src/app/App.tsx"),
  read("src/mcheyne/TodayScreen.tsx"),
  read("src/mcheyne/PlanScreen.tsx"),
  read("src/history/HistoryScreens.tsx"),
  read("src/data/schema.ts"),
  read("docs/PHASE_11_UX_VALIDATION.md"),
]);
...
assert.match(pwa, /setOffline\(true\)/); assert.match(pwa, /navigator\.serviceWorker\.controller/); assert.match(pwa, /John 3:16/);

assert.match(app, /main-content/);
assert.match(app, /focus\(\{ preventScroll: true \}\)/);
assert.match(today, /aria-label="Completed through date"/);
assert.match(plan, /aria-label="Completed through date"/);
assert.match(history, /aria-label="Previous month"/);
assert.match(history, /aria-label="Next month"/);
assert.match(history, /aria-label=\{`Open history for \$\{dateLabel\(date\)\}`\}/);
assert.match(schema, /DATABASE_SCHEMA_VERSION = 1/);

assert.match(phase11Doc, /Status:\s*\*\*implemented\*\*/i);
assert.match(phase11Doc, /Read.*Respond.*Pray.*Remember/is);
assert.match(phase11Doc, /Playwright/i);
assert.match(phase11Doc, /axe/i);
assert.match(phase11Doc, /320/i);
assert.match(phase11Doc, /200%/i);
assert.match(phase11Doc, /offline/i);
assert.match(phase11Doc, /Phase 12 — Release Hardening/i);
assert.match(phase11Doc, /npm run verify:phase11/);
...
console.log("  controlled cold-offline PWA Scripture + search journey certified");
