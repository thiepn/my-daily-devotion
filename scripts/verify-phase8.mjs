...
for (const route of ["/history", "/history/moments", "/history/day/:localDate", "/search", "/data"]) assert.ok(app.includes(route), `Missing Phase 8 route ${route}`); assert.doesNotMatch(app, /HistoryVisual/); assert.ok(main.includes("phase8.css")); assert.ok(pkg.scripts["verify:phase8"]); assert.equal(pkg.version, "0.8.0");
