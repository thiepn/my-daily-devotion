...
const [visualRaw, tokens, base, shell, screens, scripture, phase4, phase5, phase6, app, main] = await Promise.all([
  read("canonical/visual-system.v1.json"), read("src/styles/tokens.css"), read("src/styles/base.css"), read("src/styles/shell.css"), read("src/styles/screens.css"), read("src/styles/scripture.css"), read("src/styles/phase4.css"), read("src/styles/phase5.css"), read("src/styles/phase6.css"), read("src/app/App.tsx"), read("src/main.tsx"),
]);
...
const css = [tokens, base, shell, screens, scripture, phase4, phase5, phase6].join("\n");
...
for (const importPath of ["tokens.css", "base.css", "shell.css", "screens.css", "scripture.css", "phase4.css", "phase5.css", "phase6.css"]) assert.ok(main.includes(importPath), `main.tsx must import ${importPath}`);
assert.ok(!main.includes("foundation.css"));
console.log("✓ Phase 2 visual-system verification passed");
console.log("  quiet editorial visual direction frozen");
console.log("  authored light/dark/system theme support");
console.log("  no remote fonts, gradients, or default shadows");
console.log("  responsive desktop rail + mobile bottom navigation");
