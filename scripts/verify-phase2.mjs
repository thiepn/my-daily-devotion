...
const [visualRaw, tokens, base, shell, screens, scripture, phase4, phase5, app, main] = await Promise.all([
  read("canonical/visual-system.v1.json"), read("src/styles/tokens.css"), read("src/styles/base.css"), read("src/styles/shell.css"), read("src/styles/screens.css"), read("src/styles/scripture.css"), read("src/styles/phase4.css"), read("src/styles/phase5.css"), read("src/app/App.tsx"), read("src/main.tsx"),
]);
...
const css = [tokens, base, shell, screens, scripture, phase4, phase5].join("\n");
...
for (const importPath of ["tokens.css", "base.css", "shell.css", "screens.css", "scripture.css", "phase4.css", "phase5.css"]) assert.ok(main.includes(importPath), `main.tsx must import ${importPath}`);
