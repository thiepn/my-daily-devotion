...
function blockLevel(marker = "") {
  const match = marker.match(/(\d+)$/);
  return match ? Number(match[1]) : 1;
}

function sidToVerseKey(sid) {
  if (typeof sid !== "string") return null;
  const match = /^([1-3]?[A-Z]{2,3})\s+(\d+):(\d+)/.exec(sid);
...
function appendSegment(segments, segment) {
  ...
  if (
    ...
    previous.verseKey === segment.verseKey &&
    previous.redLetter === segment.redLetter &&
...
export function normalizeUsjBook(usj, bookMeta) {
  if (!usj || usj.type !== "USJ" || !Array.isArray(usj.content)) {
    throw new Error(`Invalid USJ payload for ${bookMeta.id}`);
  }

  const bookNode = usj.content.find((node) => node?.type === "book");
  if (!bookNode || bookNode.code !== bookMeta.id) {
    throw new Error(`Book identity mismatch: expected ${bookMeta.id}, received ${bookNode?.code ?? "unknown"}`);
  }

  const chapters = [];
  let chapter = null;
  let currentVerseKey = null;
  let nextIsVerseStart = false;

  const beginChapter = (node) => {
    const number = Number(node.number);
    if (!Number.isInteger(number) || number < 1) return;
    chapter = { chapter: number, blocks: [], notes: [], verseCount: 0 };
  ...
  for (const item of chapters) {
    ...
    for (const block of item.blocks) {
      for (const segment of block.segments) {
        if (segment.verseKey) keys.add(segment.verseKey);
      }
    }
    item.verseCount = keys.size;
  }

  if (chapters.length === 0) throw new Error(`${bookMeta.id} contains no chapters`);

  return {
    schemaVersion: NORMALIZED_DATA_VERSION,
    translationId: "BSB",
    bookId: bookMeta.id,
    name: bookMeta.name,
...
async function downloadPinnedSource(source) {
  ...
  const urls = [mirror.download, source.source.preferredDownload];
  let lastError = null;
  for (const url of urls) {
    try {
      ...
      if (digest !== mirror.sha256) {
        throw new Error(`SHA-256 mismatch for ${url}: expected ${mirror.sha256}, received ${digest}`);
      }
      return { bytes, url, digest };
    } catch (error) {
      lastError = error;
    }
  }
...
export async function buildBibleAssets({ force = false } = {}) {
  const [canon, source] = await Promise.all([
  ...
  if (!force && await alreadyBuilt(expectedSha)) {
    console.log("✓ BSB normalized assets already match pinned source");
    return;
  }

  const downloaded = await downloadPinnedSource(source);
  const archive = unzipSync(downloaded.bytes);
  const filesByBook = new Map();

  for (const [filename, bytes] of Object.entries(archive)) {
    if (!/\.usj$/i.test(filename)) continue;
    const usj = JSON.parse(strFromU8(bytes));
    const code = usj.content?.find((node) => node?.type === "book")?.code;
    if (typeof code === "string") filesByBook.set(code, usj);
  }

  if (filesByBook.size !== 66) {
    throw new Error(`Expected 66 USJ books, found ${filesByBook.size}`);
  }
  ...
  for (const meta of canon.books) {
    const usj = filesByBook.get(meta.id);
    if (!usj) throw new Error(`Pinned archive is missing ${meta.id}`);
    const normalized = normalizeUsjBook(usj, meta);
    const bookPath = `/bible/books/${meta.id}.json`;
    await writeFile(join(BOOKS_DIR, `${meta.id}.json`), JSON.stringify(normalized), "utf8");
    totalVerses += normalized.chapters.reduce((sum, item) => sum + item.verseCount, 0);
    manifestBooks.push({
      order: meta.order,
      id: meta.id,
      name: meta.name,
  ...
  if (totalVerses < 30000) {
    throw new Error(`Normalized verse count is unexpectedly low: ${totalVerses}`);
  }
  ...
  const manifest = {
    ...
    normalizedDataVersion: NORMALIZED_DATA_VERSION,
    generatedAt: new Date().toISOString(),
    source: {
      canonicalDownload: source.source.preferredDownload,
      downloadedFrom: downloaded.url,
      release: source.source.deterministicMirror.release,
  ...
  console.log(`✓ Built BSB Scripture assets: ${manifestBooks.length} books, ${totalVerses} verses`);
}

const invoked = process.argv[1] ? pathToFileURL(resolve(process.argv[1])).href : "";
if (import.meta.url === invoked) {
  buildBibleAssets({ force: process.argv.includes("--force") }).catch((error) => {
