...
export function TodayScreen() {
  ...
  const followCalendar = async () => {
    if (!plan) return;
    const startSequence = sequenceOnOrAfter(plan, today) ?? 365;
    await repository.enrollCalendar(today, startSequence);
    await refresh();
  };
  ...
  const importExistingProgress = async () => {
    ...
    if (!through) return;
    const next = await repository.enrollCalendar(today, 1);
    await repository.bulkImportThrough(next.id, through);
    await refresh();
  };
    ...
    return (
      <main className="visual-screen today-screen">
        <header className="screen-heading compact-heading">
          ...
          <h1>Today</h1>
          <p className="screen-intro">Begin the M’Cheyne plan without turning earlier days into artificial debt. You can follow the calendar from today, start at Day 1, or import progress you already completed this year.</p>
        </header>
        ...
        <section className="mcheyne-setup" aria-labelledby="setup-heading">
          ...
          <div className="setup-options">
            <button type="button" className="primary-editorial-action" onClick={() => void followCalendar()}>
              <span><strong>Follow today’s calendar</strong><small>Start tracking from {formatDate(today)}. Earlier readings are not marked missed.</small></span><Icon name="arrow" />
            </button>
            <button type="button" className="editorial-option" onClick={() => void startSelfPaced()}>
              <strong>Start self-paced at Day 1</strong><span>Move forward only when each day’s four readings are complete.</span>
            </button>
          </div>

          <div className="existing-progress">
            <div><p className="section-kicker">Already following this year?</p><h3>Import completed-through date</h3><p>This changes plan state only. It does not fabricate devotional-history events.</p></div>
            <div className="import-controls">
              ...
              <button type="button" onClick={() => void importExistingProgress()}>Import through this date</button>
            </div>
          </div>
        </section>
      </main>
  ...
  return (
    <main className="visual-screen today-screen">
      <header className="screen-heading compact-heading">
        <p className="eyebrow">{formatDate(today)}</p>
        <h1>Today</h1>
        <p className="screen-intro">{enrollment.mode === "CALENDAR" ? "Today stays anchored to the historic calendar. Read ahead if useful; earlier unread passages remain available without becoming a streak or score." : "Self-paced mode follows the next incomplete M’Cheyne assignment rather than the civil calendar."}</p>
      </header>
      ...
      <section className="editorial-section reading-section" aria-labelledby="reading-heading">
        <div className="section-heading-line">
          ...
          {assignment ? <span className="progress-copy">{completedCount} of 4</span> : null}
        </div>
        ...
        {isLeapPause ? (
          ...
          <div className="completion-note"><strong>All 365 M’Cheyne assignments are complete.</strong><p>Your reading history remains available in the plan and, later, in History.</p></div>
        ) : assignment ? (
          <>
            <div className="reading-list live-reading-list">
              {assignment.readings.map((reading, index) => {
                const done = Boolean(progress.get(key(assignment.sequence, index))?.completedAt);
                return (
                  <div className={`reading-row${done ? " is-complete" : ""}`} key={`${assignment.sequence}-${index}`}>
                    <button className="reading-state reading-toggle" type="button" aria-pressed={done} aria-label={`${done ? "Mark unread" : "Mark complete"}: ${reading.displayReference}`} onClick={() => void toggleReading(index)}>
                      {done ? <Icon name="check" /> : <span />}
                    </button>
                    <span className="reading-group">{reading.group === "family" ? "Family" : "Private"}</span>
                    <Link className="reading-passage" to={buildPlanReadingUrl(reading, enrollment.id, assignment.sequence, index, "today")}><strong>{reading.displayReference}</strong></Link>
            ...
            {completedCount === 4 ? <p className="factual-completion">Today’s M’Cheyne readings are complete.</p> : firstIncompleteIndex !== null ? (
              <Link className="future-action" to={buildPlanReadingUrl(assignment.readings[firstIncompleteIndex]!, enrollment.id, assignment.sequence, firstIncompleteIndex, "today")}>Continue reading <Icon name="arrow" /></Link>
            ) : null}
          </>
        ) : null}
        ...
        <div className="plan-links-row">
          ...
          {earlierUnread.length > 0 ? <span>{unreadReadingCount} earlier unread {unreadReadingCount === 1 ? "reading" : "readings"} across {earlierUnread.length} {earlierUnread.length === 1 ? "day" : "days"}</span> : <span>No earlier unread assignments in your tracked range.</span>}
        </div>
      </section>

      {earlierUnread.length > 0 ? (
        <section className="earlier-unread" aria-labelledby="earlier-heading">
          <div><p className="section-kicker">Earlier unread</p><h2 id="earlier-heading">Available when you want them.</h2><p>These stay visible because they are unread, not because you owe a streak.</p></div>
          <div className="earlier-list">{earlierUnread.slice(-4).map((item) => <Link key={item.assignment.sequence} to="/today/plan"><span>Day {item.assignment.sequence}</span><strong>{4 - item.completedCount} unread</strong><Icon name="arrow" /></Link>)}</div>
        </section>
      ) : null}

      <div className="today-lower-grid future-devotional-tools">
        <section className="editorial-section"><p className="section-kicker">Respond</p><h2>Reflection stays optional.</h2><p className="muted-copy">The Scripture-to-reflection workflow arrives in Phase 5. Today does not invent journal content before that system exists.</p></section>
        <section className="editorial-section"><p className="section-kicker">Pray</p><h2>Prayer follows reading.</h2><p className="muted-copy">Focused prayer becomes functional in Phase 6. Reading completion remains factual and independent from prayer or reflection.</p></section>
      </div>
    </main>
  );
}
