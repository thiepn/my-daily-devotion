...
export function PeopleScreen() {
  ...
  const remove = async (item: Person) => { if (!await confirmDrafts()) return; if (!await confirm({ title: `Remove ${item.name}?`, message: "Remove this person from your Prayer people list?", confirmLabel: "Remove person", danger: true })) return; await repository.removePerson(item.id); if (editing?.id === item.id) clear(); await refresh(); };
  return <main className="visual-screen metadata-screen mg-secondary-screen mg-prayer-metadata-workspace">
    ...
    </section></div>{draftDialog}{confirmDialog}
  </main>;
}
