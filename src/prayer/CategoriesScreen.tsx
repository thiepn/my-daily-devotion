...
export function CategoriesScreen() {
  ...
  const remove = async (item: Category) => { if (!await confirmDrafts()) return; if (!await confirm({ title: "Remove category?", description: `Remove ${item.name} from your Prayer categories?`, confirmLabel: "Remove category", cancelLabel: "Keep category", tone: "danger" })) return; await repository.removeCategory(item.id); if (editing?.id === item.id) clear(); await refresh(); };
