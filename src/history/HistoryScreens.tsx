...
function Heading({ title, url, children }: { title: string; url: string; children?: ReactNode }) { return <header className="history-journal-heading"><div>{children}<h1>{title}</h1></div><Link id="history-search" className="history-search" aria-label="Search history and Scripture" to={withHistoryReturn('/search', url)}><DevotionalIcon name="search" /></Link></header>; }
...
function ReflectionBand({ url }: { url: string }) {
  ...
  return <aside className="history-reflection-band" aria-label="Scripture for reflection"><MorningGraceArtwork variant="reflection" />{quote ? <blockquote>“{quote}”</blockquote> : <p>A quiet place to remember His faithfulness.</p>}<Link id="history-scripture" to={withHistoryReturn('/bible/LAM/3?verse=22', url)}>Lamentations 3:22–23 · BSB</Link></aside>;
}
...
export function HistoryScreen() {
  ...
  return <main className="grace-history history-screen"><Heading title="History" url={url} />
    <nav className="history-journal-tabs" aria-label="History views">{historyViews.map(view => { const next = new URLSearchParams(params); next.set('view', view); next.delete('shown'); return <Link id={`history-view-${view}`} key={view} aria-current={query.view === view ? 'page' : undefined} to={`/history?${next}`}>{viewNames[view]}</Link>; })}</nav>
    ...
    {error && <State error={error} retry={retry} />}<section aria-labelledby="history-activity-heading"><div className="history-activity-heading"><h2 id="history-activity-heading">{query.view === 'overview' ? 'Recent activity' : viewNames[query.view]}</h2><div><Link id="history-browse-dates" to={withHistoryReturn(`/history/calendar?period=${query.period}`, url)}>Browse dates</Link><Link id="history-moments" to={withHistoryReturn(`/history/moments?period=${query.period}`, url)}>Moments</Link></div></div>
...
export function HistoryMomentsScreen() {
  ...
  return <main className="grace-history history-moments-screen"><Heading title="Moments" url={url}><Link id="history-back" className="history-back" to={safeHistoryReturn(params.get('return'))}><DevotionalIcon name="back" />History</Link></Heading><p className="history-intro">Reflections, answers, encouragement, and Scripture to return to.</p><Period query={query} years={data?.years ?? []} today={localDate} onChange={value => change('period', value)} />{error && <State error={error} retry={retry} />}{!data && !error ? <State>Opening moments…</State> : data && <>{data.rows.length ? <Feed rows={data.rows} today={localDate} url={url} manifest={manifest} /> : <State>Reflections, answered prayers, encouragement, and highlights from this period will appear here.</State>}<div className="history-list-footer"><span role="status">Showing {data.rows.length} of {data.total} moments</span>{data.total > data.rows.length && <button id="history-show-more" type="button" onClick={() => change('shown', String(query.shown + 20))}>Show more<DevotionalIcon name="down" /></button>}</div></>}</main>;
}
function activityKinds(summary: HistoryDaySummary) {
  const counts = summary.counts;
  return [counts.READING_COMPLETED || counts.HIGHLIGHT_CREATED ? 'Scripture' : '', counts.REFLECTION_CREATED ? 'Reflection' : '', counts.PRAYER_CREATED || counts.PRAYER_PRAYED || counts.PRAYER_UPDATED || counts.ENCOURAGEMENT_RECORDED ? 'Prayer' : '', counts.PRAYER_ANSWERED ? 'Answer' : ''].filter(Boolean);
}
function markers(summary: HistoryDaySummary) { return <span className="history-date-marks" aria-hidden="true">{activityKinds(summary).map(kind => <i key={kind} className={kind.toLowerCase()} />)}</span>; }
