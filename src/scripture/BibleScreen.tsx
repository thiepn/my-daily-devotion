...
export function BibleScreen() {
  ...
  return (
    <main className="visual-screen bible-screen">
      <header className="screen-heading bible-screen-heading"><p className="eyebrow">Scripture reader · BSB</p><h1>Bible</h1><p className="screen-intro">Read the complete Berean Standard Bible in a quiet, semantic reader. Highlights, bookmarks, reflections and verse notes stay local to this device.</p></header>
      ...
      {selection && selectedReference ? <div className="verse-action-dock" role="region" aria-label={`Actions for ${referenceLabel(activeBook, routeChapter, selection)}`}><div className="selection-reference"><strong>{referenceLabel(activeBook, routeChapter, selection)}</strong><button type="button" className="clear-selection" onClick={() => { setSelection(null); setNoteEditorOpen(false); }} aria-label="Clear verse selection">×</button></div><div className="verse-actions"><button type="button" onClick={() => void toggleHighlight()}>{exactHighlight ? "Remove highlight" : "Highlight"}</button><button type="button" onClick={openReflection}><Icon name="note" /> Reflect</button><button type="button" disabled title="Prayer capture arrives in Phase 6"><Icon name="prayer" /> Pray</button><button type="button" onClick={() => void toggleBookmark()}><Icon name="bookmark" /> {exactBookmark ? "Remove bookmark" : "Bookmark"}</button><button type="button" aria-expanded={moreOpen} onClick={() => { setMoreOpen((value) => !value); setNoteEditorOpen(false); }}>More</button></div>{moreOpen ? <div className="verse-action-menu"><button type="button" onClick={() => void copySelection()}>Copy selection</button><button type="button" onClick={() => void openVerseNote()}>{exactVerseNote ? "Edit verse note" : "Add verse note"}</button></div> : null}{noteEditorOpen ? <div className="verse-note-editor"><textarea value={noteBody} onChange={(event) => setNoteBody(event.target.value)} placeholder="A durable note attached to this Scripture passage." aria-label="Verse note" /><div className="verse-note-editor-actions"><button type="button" disabled={!noteBody.trim()} onClick={() => void saveVerseNote()}>Save note</button>{exactVerseNote ? <button type="button" onClick={() => void removeVerseNote()}>Remove note</button> : null}<button type="button" onClick={() => setNoteEditorOpen(false)}>Close</button></div></div> : null}</div> : null}
