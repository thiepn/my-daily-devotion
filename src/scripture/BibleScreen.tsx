...
export function BibleScreen() {
  ...
  const discardNote = async () => {
    if (!noteEditorOpen || noteBody === savedNoteBody) return true;
    return confirm({
      title: "Discard verse note?",
      description: "Your unsaved verse note will be cleared.",
  ...
  const chooseVerse=async(verse:number)=>{if(!await discardNote())return;setStatus("");setMoreOpen(false);setNoteEditorOpen(false);setSelection((current)=>{if(!current)return{start:verse,end:verse};if(current.start===verse&&current.end===verse)return null;return{start:current.start,end:verse};});};
      ...
      {selection && selectedReference ? (
        <div className="verse-action-dock mg-verse-action-dock" role="region" aria-label={`Actions for ${referenceLabel(activeBook, routeChapter, selection)}`}>
          <div className="selection-reference">
            ...
            <button type="button" className="clear-selection" onClick={() => void discardNote().then((discard) => { if (!discard) return; setSelection(null); setNoteEditorOpen(false); })} aria-label="Clear verse selection">×</button>
          </div>
          ...
          {noteEditorOpen ? (
            <div className="verse-note-editor">
              ...
              <div className="verse-note-editor-actions">
                ...
                <button type="button" onClick={() => void discardNote().then((discard) => { if (discard) setNoteEditorOpen(false); })}>Close</button>
              </div>
            </div>
          ) : null}
        </div>
