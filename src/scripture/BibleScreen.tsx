...
export function BibleScreen() {
  ...
  const confirmDiscardNote=()=>!noteEditorOpen||noteBody===savedNoteBody?Promise.resolve(true):confirm({ title: "Discard verse note?", message: "Your unsaved verse note will be discarded.", confirmLabel: "Discard note", cancelLabel: "Keep editing", tone: "danger" });
  ...
  if((!manifest&&!error)||(!activeBook&&!error))return <main className="visual-screen bible-screen mg-route-state mg-loading-state"><p className="eyebrow">Scripture reader</p><p className="bible-loading mg-inline-state">Opening the Berean Standard Bible…</p></main>;if(error)return <main className="visual-screen bible-screen mg-route-state mg-error-state"><p className="eyebrow">Scripture reader</p><h1 className="bible-error-title">Bible</h1><p className="bible-error mg-inline-state" role="alert">{error}</p><button className="quiet-button" type="button" onClick={()=>window.location.reload()}>Try again</button></main>;if(!manifest||!activeBook||!validChapter||!chapterData)return <main className="visual-screen bible-screen mg-route-state mg-loading-state"><p className="eyebrow">Scripture reader</p><p className="bible-loading mg-inline-state">Opening {activeBook?.name??"Scripture"}…</p></main>;
  ...
  const chooseVerse=async(verse:number)=>{if(!await confirmDiscardNote())return;setStatus("");setMoreOpen(false);setNoteEditorOpen(false);setSelection((current)=>{if(!current)return{start:verse,end:verse};if(current.start===verse&&current.end===verse)return null;return{start:current.start,end:verse};});};
      ...
      {selection && selectedReference ? (
        <div className="verse-action-dock mg-verse-action-dock" role="region" aria-label={`Actions for ${referenceLabel(activeBook, routeChapter, selection)}`}>
          <div className="selection-reference">
            ...
            <button type="button" className="clear-selection" onClick={() => void (async () => { if (!await confirmDiscardNote()) return; setSelection(null); setNoteEditorOpen(false); })()} aria-label="Clear verse selection">×</button>
          </div>
          <div className="verse-actions">
            ...
            <button type="button" aria-expanded={moreOpen} onClick={() => void (async () => { if (!await confirmDiscardNote()) return; setMoreOpen((value) => !value); setNoteEditorOpen(false); })()}><Icon name="more" /> More</button>
          </div>
          {moreOpen ? (
          ...
          {noteEditorOpen ? (
            <div className="verse-note-editor">
              ...
              <div className="verse-note-editor-actions">
                ...
                <button type="button" onClick={() => void (async () => { if (await confirmDiscardNote()) setNoteEditorOpen(false); })()}>Close</button>
              </div>
            </div>
          ) : null}
        </div>
