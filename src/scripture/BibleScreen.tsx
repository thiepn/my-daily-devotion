...
export function BibleScreen() {
  ...
  const activeBook = useMemo(() => manifest?.books.find((book) => book.id === routeBookId) ?? null, [manifest, routeBookId]);
  const validChapter = activeBook !== null && Number.isInteger(routeChapter) && routeChapter >= 1 && routeChapter <= activeBook.chapterCount;
  const selectedReference = useMemo(() => activeBook && validChapter && selection ? scriptureRange(activeBook.id, routeChapter, selection.start, selection.end) : null, [activeBook, routeChapter, selection, validChapter]);
  ...
  const refreshAnnotations = useCallback(async () => {
    ...
    const [nextHighlights, nextBookmarks, nextNotes] = await Promise.all([repository.listHighlightsForChapter(activeBook.id, routeChapter), repository.listBookmarksForChapter(activeBook.id, routeChapter), verseNoteRepository.listForChapter(activeBook.id, routeChapter)]);
    setHighlights(nextHighlights); setBookmarks(nextBookmarks); setVerseNotes(nextNotes);
  ...
  useEffect(() => { if (!chapterData || !activeBook) return; const key = `${activeBook.id}.${routeChapter}?${searchKey}`; if (restoredLocation.current === key) return; restoredLocation.current = key; if (planContext) { const segment = planContext.reading.references[planContext.locator.segmentIndex]; if (!segment) return; const start = parseVerseKey(segment.startVerseKey); if (start.bookId === activeBook.id && start.chapter === routeChapter) requestAnimationFrame(() => document.querySelector<HTMLElement>(`[data-verse-key="${segment.startVerseKey}"]`)?.scrollIntoView({ block: "center" })); return; } void repository.getReaderPosition(activeBook.id, routeChapter).then((position) => { if (position?.verseKey) requestAnimationFrame(() => document.querySelector<HTMLElement>(`[data-verse-key="${position.verseKey}"]`)?.scrollIntoView({ block: "center" })); }); }, [activeBook, chapterData, planContext, routeChapter, searchKey]);
  useEffect(() => { if (!chapterData || !activeBook || !("IntersectionObserver" in window)) return; const nodes = Array.from(document.querySelectorAll<HTMLElement>(".scripture-reader [data-verse-key]")); if (!nodes.length) return; const observer = new IntersectionObserver((entries) => { const visible = entries.filter((entry) => entry.isIntersecting).sort((a,b) => a.boundingClientRect.top - b.boundingClientRect.top)[0]; const verseKey = visible?.target.getAttribute("data-verse-key") as VerseKey | null; if (!verseKey) return; if (positionTimer.current !== null) window.clearTimeout(positionTimer.current); positionTimer.current = window.setTimeout(() => { void repository.saveReaderPosition(activeBook.id, routeChapter, verseKey, window.scrollY); }, 400); }, { rootMargin: "-8% 0px -72% 0px", threshold: 0 }); nodes.forEach((node) => observer.observe(node)); return () => { observer.disconnect(); if (positionTimer.current !== null) window.clearTimeout(positionTimer.current); }; }, [activeBook, chapterData, routeChapter]);

  if (!manifest || (!activeBook && !error)) return <main className="visual-screen bible-screen"><p className="eyebrow">Scripture reader</p><p className="bible-loading">Opening the Berean Standard Bible…</p></main>;
  if (error) return <main className="visual-screen bible-screen"><p className="eyebrow">Scripture reader</p><h1 className="bible-error-title">Bible</h1><p className="bible-error" role="alert">{error}</p></main>;
  if (!activeBook || !validChapter || !chapterData) return <main className="visual-screen bible-screen"><p className="eyebrow">Scripture reader</p><p className="bible-loading">Opening {activeBook?.name ?? "Scripture"}…</p></main>;
  ...
  const exactVerseNote = selectedReference ? verseNotes.some((item) => rangeEquals(item, selectedReference)) : false;
  ...
  const chooseVerse = (verse: number) => { setStatus(""); setMoreOpen(false); setNoteEditorOpen(false); setSelection((current) => { if (!current) return { start: verse, end: verse }; if (current.start === verse && current.end === verse) return null; return { start: current.start, end: verse }; }); };
  const toggleHighlight = async () => { if (!selectedReference) return; await repository.toggleHighlight(selectedReference); await refreshAnnotations(); setStatus(exactHighlight ? "Highlight removed." : "Highlighted locally."); };
  const toggleBookmark = async () => { if (!selectedReference) return; await repository.toggleBookmark(selectedReference); await refreshAnnotations(); setStatus(exactBookmark ? "Bookmark removed." : "Bookmarked locally."); };
  const togglePlanCompletion = async () => { if (!planContext) return; await mcheyneRepository.setReadingCompleted(planContext.enrollment.id, planContext.locator.sequence, planContext.locator.readingIndex, !planContext.completed); setPlanContext({ ...planContext, completed: !planContext.completed }); setStatus(planContext.completed ? "Reading marked unread." : "Reading marked complete."); };
  const copySelection = async () => { if (!selection) return; const text = joinSelectedText(chapterData, selection); const label = referenceLabel(activeBook, routeChapter, selection); await navigator.clipboard.writeText(`${text}\n\n${label} — Berean Standard Bible`); setStatus("Selection copied."); setMoreOpen(false); };
  const openVerseNote = async () => { if (!selectedReference) return; const note = await verseNoteRepository.getExact(selectedReference); setNoteBody(note?.bodyMd ?? ""); setNoteEditorOpen(true); setMoreOpen(false); };
  const saveVerseNote = async () => { if (!selectedReference) return; try { await verseNoteRepository.save(selectedReference, noteBody); await refreshAnnotations(); setStatus("Verse note saved locally."); } catch (reason) { setStatus(reason instanceof Error ? reason.message : "Could not save verse note."); } };
  const removeVerseNote = async () => { if (!selectedReference) return; await verseNoteRepository.remove(selectedReference); setNoteBody(""); setNoteEditorOpen(false); await refreshAnnotations(); setStatus("Verse note removed from current views."); };
  const openReflection = () => { if (!selectedReference) return; navigate(buildReflectionUrl(todayLocalDate(), selectedReference, `${location.pathname}${location.search}`)); };
  const openPrayer = () => { if (!selectedReference) return; navigate(buildPrayerFromScriptureUrl(selectedReference, `${location.pathname}${location.search}`)); };
  ...
  const renderBlock = (block: ScriptureBlock, blockIndex: number) => {
    ...
    const content = block.segments.map((segment, segmentIndex) => {
      ...
      const noted = verse !== null && verseNotes.some((item) => rangeContainsVerse(item, activeBook.id, routeChapter, verse));
      ...
      const classes = ["scripture-run", segment.redLetter ? "is-red-letter" : "", highlighted ? "is-highlighted" : "", selected ? "is-selected" : "", planReading ? "is-plan-reading" : "", segment.emphasis ? `emphasis-${segment.emphasis}` : ""].filter(Boolean).join(" ");
      return <span className={classes} key={`${blockIndex}-${segmentIndex}`}>{segment.isVerseStart && segment.verseKey && verse !== null ? <button className={`verse-number${bookmarked ? " is-bookmarked" : ""}${noted ? " is-noted" : ""}`} type="button" data-verse-key={segment.verseKey} aria-label={`Select ${activeBook.name} ${routeChapter}:${verse}`} aria-pressed={selected} onClick={() => chooseVerse(verse)}>{verse}</button> : null}<span className="scripture-text">{segment.text}</span></span>;
    });
  ...
  return (
    <main className="visual-screen bible-screen">
      ...
      {selection && selectedReference ? <div className="verse-action-dock" role="region" aria-label={`Actions for ${referenceLabel(activeBook, routeChapter, selection)}`}><div className="selection-reference"><strong>{referenceLabel(activeBook, routeChapter, selection)}</strong><button type="button" className="clear-selection" onClick={() => { setSelection(null); setNoteEditorOpen(false); }} aria-label="Clear verse selection">×</button></div><div className="verse-actions"><button type="button" onClick={() => void toggleHighlight()}>{exactHighlight ? "Remove highlight" : "Highlight"}</button><button type="button" onClick={openReflection}><Icon name="note" /> Reflect</button><button type="button" onClick={openPrayer}><Icon name="prayer" /> Pray</button><button type="button" onClick={() => void toggleBookmark()}><Icon name="bookmark" /> {exactBookmark ? "Remove bookmark" : "Bookmark"}</button><button type="button" aria-expanded={moreOpen} onClick={() => { setMoreOpen((value) => !value); setNoteEditorOpen(false); }}>More</button></div>{moreOpen ? <div className="verse-action-menu"><button type="button" onClick={() => void copySelection()}>Copy selection</button><button type="button" onClick={() => void openVerseNote()}>{exactVerseNote ? "Edit verse note" : "Add verse note"}</button></div> : null}{noteEditorOpen ? <div className="verse-note-editor"><textarea value={noteBody} onChange={(event) => setNoteBody(event.target.value)} placeholder="A durable note attached to this Scripture passage." aria-label="Verse note" /><div className="verse-note-editor-actions"><button type="button" disabled={!noteBody.trim()} onClick={() => void saveVerseNote()}>Save note</button>{exactVerseNote ? <button type="button" onClick={() => void removeVerseNote()}>Remove note</button> : null}<button type="button" onClick={() => setNoteEditorOpen(false)}>Close</button></div></div> : null}</div> : null}
