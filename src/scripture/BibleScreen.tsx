    ...
    <main ref={readerRoot} className="bible-screen grace-bible" data-reading-font={readerAppearance.appearance.font} style={readerStyle}>
      ...
      {selection && selectedReference ? (
        <div ref={actionDock} className="verse-action-dock mg-verse-action-dock" role="region" aria-label={`Actions for ${referenceLabel(activeBook, routeChapter, selection)}`}>
          <div className="selection-reference"><strong>{referenceLabel(activeBook, routeChapter, selection)}</strong><button type="button" className="reader-icon-button clear-selection" onClick={() => { if (!discardNote()) return; setSelection(null); setNoteEditorOpen(false); }} aria-label="Clear verse selection"><DevotionalIcon name="close" /></button></div>
          <div className="verse-actions">
            ...
            <button type="button" disabled={annotationBusy} aria-label={exactVerseNote ? "Edit verse note" : "Add verse note"} onClick={() => { if (!discardNote()) return; void runMutation(() => openVerseNote()); }}><DevotionalIcon name="note" active={exactVerseNote} /><span>Note</span></button>
            <button type="button" aria-label="Copy selection" onClick={() => void copySelection()}><DevotionalIcon name="copy" /><span>Copy</span></button>
            <button type="button" aria-expanded={moreOpen} aria-controls="verse-more-actions" onClick={() => { if (!discardNote()) return; setMoreOpen(value => !value); setNoteEditorOpen(false); }}><DevotionalIcon name="more" /><span>More</span></button>
          </div>
          ...
          {noteEditorOpen ? <div className="verse-note-editor">
            <label htmlFor="reader-verse-note">A note on {referenceLabel(activeBook, routeChapter, selection)}</label>
            <textarea id="reader-verse-note" value={noteBody} onChange={event => setNoteBody(event.target.value)} placeholder="What do you want to remember?" aria-label="Verse note" />
            <div className="verse-note-editor-actions">
              ...
              <button type="button" onClick={() => { if (discardNote()) setNoteEditorOpen(false); }}>Close</button>
            </div>
          </div> : null}
