...
function withReturn(pathname: string, returnTo: string): string {
  return `${pathname}?${new URLSearchParams({ return: returnTo }).toString()}`;
}
...
function RouteViewportManager() {
  ...
  const restoreScroll = Boolean((location.state as { restoreScroll?: boolean } | null)?.restoreScroll);
  ...
  }, [location.pathname, navigationType, restoreScroll]);
...
export function App() {
  ...
  const searchHref = withReturn("/search", utilityReturn);
  const dataHref = withReturn("/data", utilityReturn);
  const shellClass = [
    "app-shell",
    "morning-grace-shell",
