...
function mobileRouteMeta(pathname: string): MobileRouteMeta | null {
  if (pathname === "/today/plan") return { backTo: "/today" };
  if (pathname.startsWith("/today/reflection/")) return { backTo: "/today" };
  if (pathname === "/bible/collections") return { backTo: "/bible" };
  if (pathname === "/prayer/new") return { backTo: "/prayer" };
  if (pathname === "/prayer/people") return { backTo: "/prayer" };
  if (pathname === "/prayer/categories") return { backTo: "/prayer" };
  if (pathname === "/prayer/session") return { backTo: "/prayer", immersive: true };
  const prayerSettings = pathname.match(/^\/prayer\/([^/]+)\/settings$/);
  if (prayerSettings) return { backTo: `/prayer/${prayerSettings[1]}` };
  if (/^\/prayer\/[^/]+$/.test(pathname)) return { backTo: "/prayer" };
  if (pathname === "/history/moments") return { backTo: "/history" };
  if (pathname.startsWith("/history/day/")) return { backTo: "/history" };
  if (pathname === "/search") return { backTo: "/today" };
  if (pathname === "/data") return { backTo: "/today" };
  return null;
}
